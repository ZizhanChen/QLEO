...
//zizhan 
void GLESRenderEngine::writeHIST(const char* basename, GLuint width, GLuint height){
    if (mSample != 0) {
        mSample = mSample - 1;
        ALOGE("writeHIST called, mSample: %d", mSample);
        return;
    }
    else {
        mSample = 60; 
        ...
        int fd = open(basename, O_RDWR|O_TRUNC|O_CREAT, 0666);
        if (fd >= 0) {
            auto ptr = reinterpret_cast<char*>(pixels.data());
            ...
            for (int y = height - 1; y >= 0; y--) {
                ...
                for (GLuint x = 0; x < width; x++) {
                    int iGrey = (data[0] + data[1] + data[2])/3;
                    auto itr = histogram.find(iGrey);
            ...
            for (auto it = histogram.begin(); it != histogram.end(); it++) {
                ...
                ALOGE("semantic content write: %s, ret = %d\n", buffer, amt);
            } 
            close(fd);
        }
        else {
            ALOGE("semantic content open failed: %d", fd);
        }
    }
}
...
//zizhan 
void GLESRenderEngine::writeRGB(const char* basename, GLuint width, GLuint height){
    if (mSample != 0) {
        mSample = mSample - 1;
        ALOGE("writeHIST called, mSample: %d", mSample);
        return;
        ...
        if (fd >= 0) {
            ...
            float tmp_array[width * height];
            int i = 0;
            for (int y = height - 1; y >= 0; y--) {
                char* data = ptr + y * width * sizeof(uint32_t);
                for (GLuint x = 0; x < width; x++) {
                    //int iGrey = (data[0] + data[1] + data[2])/3;
                    float brightness = sqrt(0.241*pow(data[0], 2) + 0.691*pow(data[1], 2) + 0.068*pow(data[2], 2));
                    tmp_array[i] = brightness;
                    i++;
                    data += sizeof(uint32_t);
                }
            }
            ...
            int brightness_level = (int)((avg / 255) * 14.0);
            char buffer[20];
            int bytes = sprintf(buffer, "%d\n", brightness_level);
            int amt = write(fd, buffer, bytes);
            ALOGE("semantic RGB write: %s, ret = %d\n", buffer, amt);
            close(fd);
        }
        else {
            ALOGE("semantic RGB open failed: %d", fd);
        }
    }
}
...
status_t GLESRenderEngine::drawLayers(const DisplaySettings& display,
    ...
    for (auto layer : layers) {
        ...
        writeHIST(PATH_CONTENT, mVpWidth, mVpHeight);
        writeRGB(PATH_RGB, mVpWidth, mVpHeight);
